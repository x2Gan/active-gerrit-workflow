...
def has_complete_runtime_gerrit_config(env: Mapping[str, str]) -> bool:
    if not env.get("GERRIT_BASE_URL"):
        return False
    auth_type = (env.get("GERRIT_AUTH_TYPE") or "basic").strip().lower()
    if auth_type == "basic":
        return bool(env.get("GERRIT_USERNAME") and env.get("GERRIT_HTTP_PASSWORD"))
    if auth_type == "bearer":
        return bool(env.get("GERRIT_BEARER_TOKEN"))
    if auth_type == "access_token":
        return bool(env.get("GERRIT_ACCESS_TOKEN"))
    if auth_type == "cookie_xsrf":
        return bool(env.get("GERRIT_COOKIE") and env.get("GERRIT_XSRF_TOKEN"))
    if auth_type == "anonymous":
        return True
    return True


def default_runtime_env_file(env: Mapping[str, str]) -> Path:
    configured = env.get("ACTIVE_GERRIT_WORKFLOW_ENV_FILE")
    if configured:
    ...
    return base / DEFAULT_CONFIG_SUBDIR / DEFAULT_ENV_FILENAME


def is_allowed_runtime_env_key(key: str) -> bool:
    return any(key.startswith(prefix) for prefix in RUNTIME_ENV_ALLOWED_PREFIXES)


def parse_nul_env(stdout: bytes) -> Dict[str, str]:
    parsed: Dict[str, str] = {}
    for raw_entry in stdout.split(b"\0"):
...
def source_runtime_env_file(path: Path, env: Mapping[str, str]) -> Dict[str, str]:
    bash = shutil.which("bash", path=env.get("PATH")) or "/bin/bash"
    env_bin = shutil.which("env", path=env.get("PATH")) or "/usr/bin/env"
    completed = subprocess.run(
        [bash, "-c", 'set -a; source "$1"; "$2" -0', "active-gerrit-env", str(path), env_bin],
        env=dict(env),
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        check=True,
    )
    return parse_nul_env(completed.stdout)
...
def ensure_runtime_env(env: Mapping[str, str]) -> Dict[str, str]:
    resolved = dict(env)
    env_file = default_runtime_env_file(resolved)

    if has_complete_runtime_gerrit_config(resolved):
        resolved["_ACTIVE_GERRIT_RUNTIME_ENV"] = json.dumps(
            runtime_env_status(False, env_file, attempted=False, skipped="environment_already_configured"),
            sort_keys=True,
        )
        return resolved
    ...
    if not env_file.is_file():
        resolved["_ACTIVE_GERRIT_RUNTIME_ENV"] = json.dumps(
            runtime_env_status(False, env_file, attempted=False, skipped="file_not_found"),
            sort_keys=True,
        )
        return resolved

    try:
        sourced = source_runtime_env_file(env_file, resolved)
    except (OSError, subprocess.CalledProcessError) as exc:
        resolved["_ACTIVE_GERRIT_RUNTIME_ENV"] = json.dumps(
            runtime_env_status(False, env_file, attempted=True, error=redact_message(exc, resolved)),
            sort_keys=True,
        )
        return resolved
    ...
    for key, value in sourced.items():
        if key in env or not is_allowed_runtime_env_key(key):
            continue
        resolved[key] = value
        imported.append(key)

    status = runtime_env_status(bool(imported), env_file, attempted=True)
    status["imported"] = sorted(imported)
    resolved["_ACTIVE_GERRIT_RUNTIME_ENV"] = json.dumps(status, sort_keys=True)
    return resolved


def runtime_env_meta(env: Mapping[str, str]) -> Optional[Dict[str, Any]]:
    raw = env.get("_ACTIVE_GERRIT_RUNTIME_ENV")
    if not raw:
        return None
    try:
    ...
    if isinstance(imported, list):
        value["imported"] = [key for key in imported if not key.endswith(("PASSWORD", "TOKEN", "COOKIE"))]
    if value.get("error"):
        value["error"] = redact_message(value["error"], env)
    return value
...
def base_meta(args: argparse.Namespace, env: Mapping[str, str]) -> Dict[str, Any]:
    ...
    runtime_env = runtime_env_meta(env)
    if runtime_env:
...
def environment_checks(env: Mapping[str, str]) -> Dict[str, Any]:
    auth_type = (env.get("GERRIT_AUTH_TYPE") or "basic").strip().lower()
    runtime_env = runtime_env_meta(env) or {}
    runtime_env_hint = None
    if runtime_env.get("path"):
        runtime_env_hint = (
            f"Prefer the active-gerrit launcher, or make sure {runtime_env['path']} is readable "
            "when running scripts/gerrit_cli.py directly."
        )
    checks: Dict[str, Any] = {
        "GERRIT_BASE_URL": {
            ...
            "hint": None if env.get("GERRIT_BASE_URL") else runtime_env_hint or "Set GERRIT_BASE_URL to your Gerrit Web root URL.",
        },
        "GERRIT_AUTH_TYPE": {
            "ok": True,
            "required": False,
        ...
        "GERRIT_USERNAME": {
            ...
            "hint": None if auth_type != "basic" or env.get("GERRIT_USERNAME") else runtime_env_hint or "Set GERRIT_USERNAME.",
        },
        "GERRIT_HTTP_PASSWORD": {
            ...
            else runtime_env_hint or "Set GERRIT_HTTP_PASSWORD to the Gerrit UI generated HTTP password.",
        },
    }
    if runtime_env:
        checks["runtime_env"] = {
            "ok": runtime_env.get("loaded") or runtime_env.get("skipped") in {"environment_already_configured", "file_not_found"},
            "required": False,
            "value": runtime_env,
            "hint": runtime_env_hint if runtime_env.get("error") else None,
        }
...
def current_revision_ref(change: Mapping[str, Any]) -> Optional[str]:
    current_revision = change.get("current_revision")
    revisions = change.get("revisions")
    if not current_revision or not isinstance(revisions, Mapping):
        return None
...
def normalize_updated_refs(before: Mapping[str, Any], after: Mapping[str, Any], response_data: Any) -> Sequence[str]:
    refs = []
    if isinstance(response_data, Mapping):
        for key in ("ref", "branch_ref", "destination_ref"):
            value = response_data.get(key)
            if isinstance(value, str) and value.strip():
                refs.append(value)

        ref_updates = response_data.get("ref_updates")
        if isinstance(ref_updates, Sequence) and not isinstance(ref_updates, (str, bytes, bytearray)):
        ...
        refs_raw = response_data.get("refs")
        if isinstance(refs_raw, Sequence) and not isinstance(refs_raw, (str, bytes, bytearray)):
            for item in refs_raw:
                if isinstance(item, str) and item.strip():
                    refs.append(item)

    before_ref = current_revision_ref(before)
    after_ref = current_revision_ref(after)
    if after_ref and after_ref != before_ref:
        refs.append(after_ref)
    return unique_texts(refs)


def build_high_risk_operation_plan(
    operation: str,
    change: str,
    ...
    plan: Dict[str, Any] = {
        ...
        "before": normalize_change_state_summary(detail),
        "updated_refs": [],
    }
    if extra:
        plan.update(extra)
...
def execute_high_risk_change_action(
    client: GerritClient,
    change: str,
    operation: str,
    endpoint_suffix: str,
    ...
    yes: bool,
    notify: Optional[str] = None,
    plan_extra: Optional[Mapping[str, Any]] = None,
    result_extra: Optional[Mapping[str, Any]] = None,
) -> Dict[str, Any]:
    before = change_state_detail(client, change)
    dry_run = not yes
    ...
    response = client.post(change_resource_path(change, endpoint_suffix), json_body=payload)
    after = change_state_detail(client, change)
...
def rebase_payload(args: argparse.Namespace) -> Dict[str, Any]:
    payload: Dict[str, Any] = {"allow_conflicts": bool(args.allow_conflicts)}
    if args.base is not None:
        payload["base"] = validate_optional_string(args.base, "--base")
    return payload


def abandon_payload(args: argparse.Namespace) -> Dict[str, Any]:
    return {
        "message": change_operation_message(args, required=True),
        "notify": validate_notify_with_default(args.notify, DEFAULT_ABANDON_RESTORE_NOTIFY),
    }


def restore_payload(args: argparse.Namespace) -> Dict[str, Any]:
    payload: Dict[str, Any] = {
        "notify": validate_notify_with_default(args.notify, DEFAULT_ABANDON_RESTORE_NOTIFY),
    }
    message = change_operation_message(args, required=False)
    if message is not None:
        payload["message"] = message
    return payload


def rebase_action(client: GerritClient, args: argparse.Namespace) -> Dict[str, Any]:
    payload = rebase_payload(args)
    base = payload.get("base")
    reason = f"Rebase onto {base}." if isinstance(base, str) else "Rebase onto the latest target branch tip."
    expected_effect = (
        f"Rebase would create a new patch set using {base} as the base change."
        if isinstance(base, str)
        else "Rebase would create a new patch set on top of the latest target branch revision."
    )
    return execute_high_risk_change_action(
        client,
        args.change,
        "rebase",
        "rebase",
        ...
        reason,
        expected_effect,
        ["Review the new patch set and diff after rebasing."],
        yes=bool(args.yes),
        notify=None,
    )


def abandon_action(client: GerritClient, args: argparse.Namespace) -> Dict[str, Any]:
    payload = abandon_payload(args)
    return execute_high_risk_change_action(
        client,
        args.change,
        "abandon",
        ...
        payload,
        payload["message"],
        "Abandon would move the change to ABANDONED and remove it from open review queues.",
        ["Notify reviewers or owners if follow-up is still needed after abandoning the change."],
        yes=bool(args.yes),
        notify=payload["notify"],
    )


def restore_action(client: GerritClient, args: argparse.Namespace) -> Dict[str, Any]:
    payload = restore_payload(args)
    reason = payload.get("message") or "Restore the abandoned change."
    return execute_high_risk_change_action(
        client,
        args.change,
        "restore",
        ...
        payload,
        reason,
        "Restore would return the change to NEW status and put it back in review queues.",
        ["Re-check submit requirements and reviewers after restoring the change."],
        yes=bool(args.yes),
        notify=payload["notify"],
    )
...
def validate_notify_with_default(notify: Any, default_value: str = DEFAULT_NOTIFY) -> str:
    value = default_value if notify is None else notify
...
def validate_notify(notify: Any) -> str:
    return validate_notify_with_default(notify, DEFAULT_NOTIFY)
...
def submit_action(client: GerritClient, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    planned_request = dict(plan.get("planned_request") or {})
...
def handle_rebase(args: argparse.Namespace, env: Mapping[str, str]) -> Dict[str, Any]:
    client = GerritClient.from_env(env)
    result = rebase_action(client, args)
    return success_envelope("rebase", result, args, env, warnings=result.get("warnings"))


def handle_abandon(args: argparse.Namespace, env: Mapping[str, str]) -> Dict[str, Any]:
    client = GerritClient.from_env(env)
    result = abandon_action(client, args)
    return success_envelope("abandon", result, args, env, warnings=result.get("warnings"))


def handle_restore(args: argparse.Namespace, env: Mapping[str, str]) -> Dict[str, Any]:
    client = GerritClient.from_env(env)
    result = restore_action(client, args)
    return success_envelope("restore", result, args, env, warnings=result.get("warnings"))
...
def build_parser() -> JsonArgumentParser:
    ...
    submit_parser = subparsers.add_parser("submit", help="Run Gerrit submit prechecks and optionally execute submit.")
    ...
    submit_parser.add_argument(
        "--yes",
        action="store_true",
        help="Actually submit after prechecks pass.",
    )
    submit_parser.add_argument(
        "--notify",
        choices=NOTIFY_OPTIONS,
    ...
    rebase_parser.add_argument("--base", help="Optional base change, patch set, or revision identifier.")
    rebase_parser.add_argument(
        "--allow-conflicts",
        action="store_true",
        help="Allow Gerrit to create a rebase even when conflicts are expected.",
    )
    rebase_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the rebase plan without creating a new patch set.",
    )
    rebase_parser.add_argument(
        "--yes",
        action="store_true",
        help="Actually execute the rebase operation.",
    )
    rebase_parser.set_defaults(handler=handle_rebase)

    abandon_parser = subparsers.add_parser("abandon", help="Plan or execute abandoning a Gerrit change.")
    abandon_parser.add_argument("--change", required=True, help="Gerrit change id, preferably <project>~<number>.")
    abandon_message = abandon_parser.add_mutually_exclusive_group(required=True)
    abandon_message.add_argument("--message", help="Message visible to reviewers when abandoning the change.")
    abandon_message.add_argument("--reason", help="Reason visible to reviewers when abandoning the change.")
    abandon_parser.add_argument(
        "--notify",
        choices=NOTIFY_OPTIONS,
        help="Override abandon notify mode. Defaults to OWNER.",
    )
    abandon_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the abandon plan without changing Gerrit state.",
    )
    abandon_parser.add_argument(
        "--yes",
        action="store_true",
        help="Actually abandon the change.",
    )
    abandon_parser.set_defaults(handler=handle_abandon)

    restore_parser = subparsers.add_parser("restore", help="Plan or execute restoring an abandoned Gerrit change.")
    restore_parser.add_argument("--change", required=True, help="Gerrit change id, preferably <project>~<number>.")
    restore_message = restore_parser.add_mutually_exclusive_group()
    restore_message.add_argument("--message", help="Optional message visible to reviewers when restoring the change.")
    restore_message.add_argument("--reason", help="Optional reason visible to reviewers when restoring the change.")
    restore_parser.add_argument(
        "--notify",
        choices=NOTIFY_OPTIONS,
        help="Override restore notify mode. Defaults to OWNER.",
    )
    restore_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the restore plan without changing Gerrit state.",
    )
    restore_parser.add_argument(
        "--yes",
        action="store_true",
        help="Actually restore the change.",
    )
...
def run(argv: Optional[Sequence[str]] = None, env: Optional[Mapping[str, str]] = None) -> int:
    actual_env = ensure_runtime_env(os.environ if env is None else env)
    parser = build_parser()
