...
- Agent 正常使用时调用安装器生成的 `active-gerrit` / `active-gerrit-workflow` launcher，而不是直接调用 `scripts/gerrit_cli.py`。Python 脚本是实现入口和测试入口，负责在被误直调时兜底加载运行配置。

### 1.1 运行时配置加载策略

调研主流实践后，运行配置采用以下规则：

- 配置与代码分离，最终运行时输入仍是环境变量。Gerrit URL、用户名、HTTP password、token、cookie、cache 目录等不写入代码。
- 用户级配置文件遵循 XDG 风格，默认放在 `~/.config/active-gerrit-workflow/env`；安装器生成的 launcher 会在启动前 source 这个文件。
- 环境变量优先于 env 文件。若调用进程已经设置了 `GERRIT_*`、`ACTIVE_GERRIT_*`、`GIT_*`，文件值不覆盖它们，只补齐缺失项。
- 直接执行 `python active-gerrit/scripts/gerrit_cli.py ...` 时，CLI 会在 Gerrit 配置不完整时尝试加载 `$ACTIVE_GERRIT_WORKFLOW_ENV_FILE` 或默认 env 文件，避免 Agent 绕过 launcher 后误报“未配置用户名密码”。
- 自动加载只导入受控前缀，不把整个 shell 环境扩散进 CLI；输出中只记录是否加载、文件路径和非敏感导入项，密码、token、cookie 不进入日志。

参考实践：

- [The Twelve-Factor App: Config](https://www.12factor.net/config)：配置应从代码中分离，并由环境提供。
- [XDG Base Directory Specification](https://specifications.freedesktop.org/basedir-spec/0.8/)：用户级配置默认位于 `$XDG_CONFIG_HOME`，未设置时使用 `~/.config`。
- [Docker Compose environment precedence](https://docs.docker.com/compose/how-tos/environment-variables/envvars-precedence/)：显式运行环境应高于 env file 之类的文件来源。
...
建议对 Agent 和用户只暴露一个稳定 CLI 入口：

```bash
active-gerrit <command> [options]
```

`scripts/gerrit_cli.py` 保留为内部实现入口。开发、单元测试或排障可以直接执行它；为了兼容 Agent 误直调，它会按 1.1 的规则尝试自加载运行配置。
...
本节的 REST API 请求主要固化在 `active-gerrit/scripts/gerrit_cli.py`，并通过 `active-gerrit` launcher 暴露。命令输出稳定 JSON，供 Agent 直接消费，也供 `active-gerrit-workflow` 编排复用。
...
```bash
active-gerrit query-preset my_open_reviews --limit 25
active-gerrit query-preset project_branch_open --project myProject --branch master
```
...
- 流程脚本调用 `active-gerrit`，不重复实现 REST client。
- 输出 `WorkflowReport`。
...
active-gerrit doctor
active-gerrit query-preset my_open_reviews --limit 25
active-gerrit get-change --change myProject~4247 --detail full
active-gerrit list-files --change myProject~4247 --revision current
active-gerrit get-diff --change myProject~4247 --revision current --file src/main/App.java
active-gerrit review --change myProject~4247 --revision current --input review.json
active-gerrit add-reviewer --change myProject~4247 --reviewer alice@example.com
active-gerrit submit --change myProject~4247 --dry-run
active-gerrit submit --change myProject~4247 --yes
```

`active-gerrit-workflow` 流程脚本：

```bash
active-gerrit-workflow my-review-queue --limit 25
active-gerrit-workflow review-brief --change myProject~4247
active-gerrit-workflow pre-submit-check --change myProject~4247
active-gerrit-workflow owner-report --project myProject --after 2026-05-01
active-gerrit-workflow stale-review-report --project myProject --days 7
```
