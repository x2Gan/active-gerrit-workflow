...
- TODO：
  ...
  - [ ] 校验 comments 文件路径。
  - [ ] 校验 line/range。
  - [ ] 支持 patchset-level comment `/PATCHSET_LEVEL`。
  - [ ] 默认 tag：`autogenerated:active-gerrit`。
  - [ ] 默认 notify：`OWNER_REVIEWERS`。
- 验收：
  - [ ] payload 符合 Gerrit `ReviewInput`。
  - [ ] dry-run 能输出 `ReviewPlan`。
